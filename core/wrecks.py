...
def list_wrecks(wrecks_dir: Path) -> list[dict[str, Any]]:
    return [_summary(record) for record in _load_records(wrecks_dir)]
...
def list_wreck_photo_review_items(wrecks_dir: Path) -> list[dict[str, Any]]:
    ...
    for record in _load_records(wrecks_dir):
        ...
        for photo in record.get("attached_photos") or []:
            ...
            if not photo_id:
                continue
            public_image = _wreck_public_file_url(wreck_id, photo.get("public_image_file")) if is_approved(photo) else None
            public_thumb = _wreck_public_file_url(wreck_id, photo.get("public_thumb_file")) if is_approved(photo) else None
            items.append(
                {
                    "scope": "wreck",
...
def _attached_photo_by_id(record: dict[str, Any], photo_id: str) -> dict[str, Any]:
    for photo in record.get("attached_photos") or []:
        if isinstance(photo, dict) and str(photo.get("id") or "") == photo_id:
            return photo
    raise FileNotFoundError("Nie znaleziono zdjęcia w teczce pojazdu.")
...
def public_wreck_asset(wreck_id: str, relative_path: str, wrecks_dir: Path) -> tuple[Path, str]:
    ...
    rel = str(relative_path or "").replace("\\", "/").strip("/")
    if not rel or rel.startswith("/") or any(part in {"", ".", ".."} for part in rel.split("/")):
        raise FileNotFoundError("Nie znaleziono publicznego pliku teczki pojazdu.")
...
def delete_wreck(wreck_id: str, wrecks_dir: Path) -> dict[str, Any]:
    ...
    if not record_dir.exists():
        raise FileNotFoundError("Nie znaleziono zapisanej teczki pojazdu.")
    if not (record_dir / "record.json").exists():
        raise ValueError("Katalog nie wygląda jak teczka pojazdu.")
...
def _save_attached_photo(upload: UploadedFile, record_dir: Path) -> dict[str, Any]:
...
def attach_wreck_photos(wreck_id: str, uploads: list[UploadedFile], wrecks_dir: Path) -> dict[str, Any]:
    ...
    if not isinstance(attached, list):
        attached = []
    saved = [_save_attached_photo(upload, record_dir) for upload in photo_uploads]
...
def _admin_upload_section(wreck_id: str) -> str:
    ...
    return f"""
    <section class="evidence admin-upload" data-report-admin-upload>
      <h2>Dodaj zdjęcia do teczki</h2>
      ...
      <p id="wreck-photo-status">Zaloguj się jako administrator w aplikacji, żeby dodawać zdjęcia.</p>
    </section>
    <script data-report-admin-script>
    (() => {{
      ...
      fetch('/api/admin/status', {{ cache: 'no-store' }})
        .then(resp => resp.json())
        .then(data => {{
          const ok = data && data.authenticated === true;
          setEnabled(ok);
          if (status) status.textContent = ok ? '' : 'Zaloguj się jako administrator w aplikacji, żeby dodawać zdjęcia.';
        }})
...
def _render_record_html(record: dict[str, Any], record_dir: Path) -> None:
    if _migrate_wreck_record(record_dir, record):
        _write_json(record_dir / "record.json", record)
    title = f"Teczka pojazdu {record['id']}"
    latest = record.get("latest_evidence") or {}
    attached_photos = _approved_attached_photos(record)
    evidence_sections: list[str] = []
    for evidence in reversed(record.get("evidences") or []):
        crop_cards: list[str] = []
        evidence_path = html.escape(evidence["path"])
        ...
        evidence_sections.append(
            ...
            <section class="evidence">
              ...
              <p>Rank #{evidence.get("rank")} · score {(float(evidence.get("score") or 0) * 100):.0f}% · lata: {html.escape(evidence_labels)}</p>
              <div class="grid">{"".join(crop_cards)}</div>
              <p><a href="{evidence_path}/candidate.json">candidate.json</a> · <a href="{evidence_path}/metadata.json">metadata.json</a> · <a href="{evidence_path}/links.json">links.json</a></p>
            </section>
            """
        )
...
<head>
  ...
  <style>
    ...
    .admin-upload form {{ display:flex; flex-wrap:wrap; align-items:center; gap:8px; }}
    .admin-upload input {{ max-width:360px; color:var(--mut); }}
    .admin-upload button {{ border:0; border-radius:8px; padding:9px 12px; background:#2563eb; color:white; font-weight:800; cursor:pointer; }}
    .admin-upload p {{ margin:4px 0 0; font-size:12px; }}
...
<main>
  ...
  {_admin_upload_section(record["id"])}
</main>
</body>
</html>
"""
...
def save_manual_wreck(lat: Any, lon: Any, wrecks_dir: Path) -> dict[str, Any]:
    ...
    evidence_id = _manual_evidence_id(lat_f, lon_f, created_at)
    ...
    _write_json(
        ...
        {"source": "manual_inspection", "created_at": created_at, "lat": lat_f, "lon": lon_f, "links": links},
    )
    _write_json(record_dir / "record.json", record)
    _render_record_html(record, record_dir)
...
def save_wreck_from_rank(rank: int, analysis_dir: Path, data_dir: Path, wrecks_dir: Path) -> dict[str, Any]:

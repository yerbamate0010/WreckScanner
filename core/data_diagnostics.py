    ...
    *,
    ...
    if not value:
        if record.get("private_original_deleted_at"):
            return None, 0
        _issue(issues, "error", f"{code_prefix}_missing_private_original", record_path, "Brak pola private_original_file.")
        return None, 0
...
def _audit_wreck_asset(
    ...
    *,
    ...
    if not file_path:
        _issue(issues, "error", f"{code_prefix}_unsafe_path", record_path, "Niebezpieczna ścieżka w teczce pojazdu.")
        return None, 0
    if not file_path.exists():
...
def _audit_wrecks(
    ...
    *,
    ...
    for child in sorted(wrecks_dir.iterdir()):
        ...
        if not record_path.exists():
            summary["orphan_directories"] += 1
            _issue(issues, "error", "wreck_record_missing", child, "Katalog teczki pojazdu nie ma record.json.")
            continue
        try:
            record = _read_json(record_path)
        except (OSError, json.JSONDecodeError) as exc:
        ...
        if not WRECK_ID_RE.fullmatch(wreck_id):
            _issue(issues, "error", "wreck_bad_id", record_path, "Nieprawidłowy identyfikator teczki pojazdu.", id=wreck_id)
        ...
        if not _is_coord(record.get("lat"), -90, 90) or not _is_coord(record.get("lon"), -180, 180):
            _issue(issues, "error", "wreck_bad_coordinates", record_path, "Nieprawidłowe współrzędne teczki pojazdu.")
        if not (child / "index.html").exists():
            summary["missing_index_html"] += 1
            _issue(
                issues, "warning", "wreck_index_missing", child / "index.html", "Brakuje lokalnego index.html teczki."
            )
        ...
        for photo in attached:
            ...
            if previous_wreck:
                _issue(
                    ...
                    record_path,
                    "To samo zdjęcie jest przypisane do więcej niż jednej teczki.",
                    photo_id=photo_id,
            ...
            if photo_id in field_photo_ids:
                _issue(
                    ...
                    record_path,
                    "Zdjęcie przeniesione do teczki pojazdu nadal istnieje jako luźna pinezka terenowa.",
                    photo_id=photo_id,
                )
            if (
                ...
                _issue(
                    ...
                    record_path,
                    "Do teczki pojazdu przypięto niepojazdowy typ zdjęcia.",
                )
...
def format_data_diagnostics(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Teczki pojazdów: {wrecks['records']} teczek, {wrecks['evidences']} dowodów, {wrecks['attached_photos']} zdjęć z miejsca",
        "  pliki zdjęć w teczkach: "

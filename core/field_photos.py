...
def _summary(record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "public_image": _public_file_url(photo_id, "public-image"),
        "public_thumb": _public_file_url(photo_id, "public-thumb"),
...
def list_field_photos(storage_dir: Path, *, private_dir: Path | None = None) -> list[dict[str, Any]]:
    ...
    for path in sorted(storage_dir.glob("*/record.json")):
        try:
            ...
            continue
        if isinstance(record, dict) and record.get("id") and is_approved(record):

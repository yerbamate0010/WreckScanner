...
function publicFeatureAllowed(featureKey) {
    return adminAuthenticated || publicFeatureSettings[featureKey] !== false;
}
...
function updatePublicFeatureControlVisibility() {
    ...
    if (contextFieldPhotoButton) contextFieldPhotoButton.hidden = !photoUploadsAllowed;
...
async function deleteGeotiffCacheItem(fileName, button = null) {
    ...
    try {
        const resp = await fetch(`${ADMIN_GEOTIFF_CACHE_URL}/${encodeURIComponent(file)}`, { method: 'DELETE' });
        const data = await resp.json().catch(() => ({}));
        if (!resp.ok || data.status !== 'ok') {
        ...
        if (status) status.textContent = err.message || t('modal.geotiffCache.deleteError');
    }
}
...
function pendingFieldPhotoPopup(lat, lon) {
    return `
        <div class="map-popup map-popup--pending-submission">
...
async function saveManualWreck(lat, lon, button = null) {
    ...
    try {
        const resp = await fetch(WRECKS_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ lat: latNumber, lon: lonNumber, cropM }),
        });
        const data = await resp.json().catch(() => ({}));
        if (!resp.ok || data.status !== 'ok') {
...
async function loadWreckReviewQueue() {
    ...
    try {
        ...
        const resp = await fetch(`${ADMIN_WRECKS_URL}?${params.toString()}`, { cache: 'no-store' });
        const data = await resp.json().catch(() => ({}));
        if (!resp.ok || data.status !== 'ok') {
        ...
        if (status) status.textContent = err.message || t('modal.wreckReview.loadError');
    }
}
...
async function saveWreckReviewStatus(publicReviewStatus) {
    ...
    try {
        ...
        if (status) status.textContent = err.message || t('modal.wreckReview.saveError');
    }
}
...
async function deleteWreckReviewItem() {
    ...
    try {
        ...
        if (status) status.textContent = err.message || t('wreck.deleteError');
    }
}
...
function fieldPhotoCard(photo) {
    ...
    const publicUrl = photo.public_image || photo.public_thumb || '';
    const thumbUrl = publicThumb ? `${publicThumb}?ts=${Date.now()}` : '';
    const publicImage = publicUrl ? `${publicUrl}?ts=${Date.now()}` : '';
    ...
    const photoPreview = publicImage && thumbUrl ? `
...
async function rejectFieldPhotoGroup(encodedPhotoIds, button = null) {
    ...
    try {
        for (const id of photoIds) {
            const resp = await fetch(`${ADMIN_PHOTOS_URL}/field/${encodeURIComponent(id)}/review`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ public_review_status: 'rejected', redactions: [] }),
            });
            const data = await resp.json().catch(() => ({}));
            if (!resp.ok || data.status !== 'ok') {
        ...
        if (btn) btn.disabled = false;
        statusEl.textContent = err.message || t('fieldPhoto.rejectError');
        statusEl.className = 'err';
    }
}
...
function fieldPhotoGroupPopup(group) {
    ...
    if (reviewStatus === 'pending') {
        return adminAuthenticated
            ...
            : pendingFieldPhotoPopup(group.lat, group.lon);
    }
    const photos = group.photos || [];
...
async function deletePhotoReviewItem() {
    ...
    try {
        ...
        if (status) status.textContent = err.message || t('modal.photoReview.deleteError');
    }
}
...
map.on('click', async (e) => {
    ...
    try {
        const res = await fetch('/api/inspect', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ lat: inspectLat, lon: inspectLon, cropM: selectedReviewCropM() })
        });
        const data = await res.json();

        if (!res.ok || data.status !== 'ok' || !Array.isArray(data.crops) || data.crops.length === 0) {

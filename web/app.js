...
function updateMapSourceUi() {
    ...
    currentLabel.textContent = source.label;
    currentLabel.title = source.label;
    document.getElementById('year-range').value = currentMapSourceIndex;
    document.querySelectorAll('.year-tick').forEach(t => {
        t.classList.toggle('active', Number(t.dataset.index) === currentMapSourceIndex);
    });
}
...
function setMapSource(index) {
    ...
    if (!Number.isFinite(nextIndex) || nextIndex === currentMapSourceIndex) return;
...
function setCadastralLayerVisible(visible) {
    ...
    if (toggle) toggle.checked = cadastralLayerVisible;
    try {
...
(() => {
    ...
    range.min = 0;
    range.max = MAP_SOURCES.length - 1;
    range.step = 1;
    MAP_SOURCES.forEach((source, index) => {
        const t = document.createElement('button');
        t.type = 'button';
        t.className = 'year-tick' + (index === currentMapSourceIndex ? ' active' : '');
        t.dataset.index = index;
        t.textContent = source.shortLabel;
        t.title = source.label;
        t.addEventListener('click', () => setMapSource(index));
        ticks.appendChild(t);
    });
    updateMapSourceUi();
    range.addEventListener('input', e => setMapSource(e.target.value));
    document.getElementById('year-prev').addEventListener('click', () => setMapSource(currentMapSourceIndex - 1));
    document.getElementById('year-next').addEventListener('click', () => setMapSource(currentMapSourceIndex + 1));
    document.addEventListener('keydown', e => {
        // Nie przechwytuj klawiszy gdy użytkownik pisze w polu albo gdy modal jest otwarty
        if (e.target.matches('input, select, textarea')) return;
        if (document.querySelector('.modal-backdrop:not([hidden])')) return;
        if (e.key === 'ArrowLeft') { setMapSource(currentMapSourceIndex - 1); e.preventDefault(); }
        else if (e.key === 'ArrowRight') { setMapSource(currentMapSourceIndex + 1); e.preventDefault(); }
    });
})();
...
function savedWreckPopup(wreck) {
    ...
    const deleteButton = adminAuthenticated
        ...
        : '';
    const compactLinks = [
        popupCompactLink(folder, 'Folder', t('wreck.openFolder')),
    ...
    return `
        <div class="map-popup map-popup--vehicle-case">
            ...
            ${popupActions([reportButton, photoButton, deleteButton])}
        </div>
    `;
}
...
async function uploadFieldPhotoItems(items) {
    ...
    if (!summary.failed && input) input.value = '';
    if (submit) {
        submit.disabled = false;
        submit.querySelector('span').textContent = t('modal.fieldPhoto.submit');
    }
...
function fieldPhotoGroupActions(group) {
    ...
    const deleteButton = adminAuthenticated && encodedPhotoIds
        ...
        : '';
    return popupActions([reportButton, photoButton, deleteButton]);
}
...
async function openFieldPhotoGroupPhotoUpload(lat, lon, encodedPhotoIds, button = null) {
    ...
    statusEl.className = '';
    try {
        const wreckId = await createWreckForFieldPhotoGroup(lat, lon, encodedPhotoIds);
        if (!wreckId) return;
        statusEl.textContent = t('fieldPhoto.prepareCaseSaved');
        statusEl.className = 'ok';
        openWreckPhotoModal(wreckId);
...
async function loadPhotoReviewQueue() {
    ...
    try {
        const resp = await fetch(`${ADMIN_PHOTOS_URL}?status=${encodeURIComponent(filter)}&ts=${Date.now()}`, { cache: 'no-store' });
        const data = await resp.json().catch(() => ({}));
        if (!resp.ok || data.status !== 'ok') {
...
async function runAll() {
    ...
    try {
        ...
        if (anResp.ok && anData.status === 'ok') {
            ...
            // Wiersz akcji (raport + kosz) — pojawia się tylko jeśli coś znaleźliśmy
            if (n > 0) {
                btnReport.href = reportLink;
                reportLabelEl.textContent = t(`result.${reportForm(n)}`, { n });

...
def _safe_geotiff_cache_file_name(file_name: str) -> str:
    name = str(file_name or "").strip()
    if not name or Path(name).name != name or "/" in name or "\\" in name:
        raise ValueError("Nieprawidłowa nazwa pliku GeoTIFF.")
    allowed = set("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789._-")
    if any(ch not in allowed for ch in name):
        raise ValueError("Nieprawidłowa nazwa pliku GeoTIFF.")
    if not name.lower().endswith((".tif", ".tiff", ".tif.part", ".tiff.part")):
        raise ValueError("Plik nie jest elementem cache GeoTIFF.")
    return name


def delete_geotiff_cache_file(file_name: str) -> dict[str, Any]:
    raw_dir = config.WFS_GEOTIFF_CACHE_DIR
    name = _safe_geotiff_cache_file_name(file_name)
    path = raw_dir / name
    try:
        stat = path.stat()
    except FileNotFoundError as exc:
        raise FileNotFoundError("Nie znaleziono pliku GeoTIFF w cache.") from exc
    if not path.is_file():
        raise ValueError("Wskazany element cache nie jest plikiem.")
    path.unlink()
    return {
        "status": "ok",
        "file": name,
        "size_bytes": stat.st_size,
        "summary": geotiff_cache_report(raw_dir),
    }

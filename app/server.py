...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def _public_field_photo_allowed(self, photo: dict) -> bool:
        ...
        return self._public_layer_settings().get(key, True)
    ...
    def _handle_public_wreck_asset(self, request_path: str, *, include_body: bool = True) -> None:
        ...
        if len(parts) < 3 or parts[0] != config.WRECKS_ROUTE:
            self._send_json(
                404,
                {"error": "Nie znaleziono publicznego pliku teczki pojazdu."},
    ...
    def do_DELETE(self):
        ...
        if request_path.startswith("/api/wrecks/"):
            ...
            if not wreck_id or "/" in wreck_id:
                ...
                self.send_header("Content-Type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"error": "Nieprawidłowy identyfikator teczki pojazdu."}).encode())
                return
            try:
    ...
    def _handle_get_wrecks(self) -> None:
        ...
        self.send_header("Cache-Control", "no-store")
        self.end_headers()
        wrecks = list_wrecks(core_config.WRECKS_DIR) if self._public_layer_allowed("saved_wrecks") else []
    ...
    def do_POST(self):
        ...
        if request_path == "/api/settings":
            ...
            try:
                ...
                md_path = config.DOWNLOAD_DATA_DIR / "metadata.json"
                if not md_path.exists():
                    raise ValueError("Brak pobranych danych. Wykonaj najpierw pobieranie.")
                with md_path.open(encoding="utf-8") as f:
                    md = json.load(f)

                bb = md["bbox_4326"]
                img_w = md["image_width_px"]
                img_h = md["image_height_px"]

                x = int((lon - bb["min_lon"]) / (bb["max_lon"] - bb["min_lon"]) * img_w)
                y = int((bb["max_lat"] - lat) / (bb["max_lat"] - bb["min_lat"]) * img_h)

                crop_size = config.INSPECT_CROP_PX
                half = crop_size // 2
                x1, y1 = max(0, x - half), max(0, y - half)
                x2, y2 = min(img_w, x + half), min(img_h, y + half)
                ...
                crops = []
                for year in md.get("years", []):
                    img_path = config.DOWNLOAD_DATA_DIR / f"ortofoto_{year}.png"
                    if img_path.exists():
                        img = Image.open(img_path)
                        chunk = img.crop((x1, y1, x2, y2))
                        name = f"custom_{ts}_{year}.jpg"
                        path = custom_dir / name
                        chunk.convert("RGB").save(path, "JPEG", quality=config.INSPECT_JPEG_QUALITY)
                        crops.append({"year": year, "url": f"/{config.ANALYSIS_DIR_NAME}/custom_crops/{name}"})

                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"status": "ok", "crops": crops}).encode())
            ...
            try:
                ...
                if data.get("fast") is True:
                    cmd.append("--fast")
                try:
                    crop_m = float(data.get("cropM", core_config.REVIEW_CROP_M))
                except (TypeError, ValueError):
                    raise ValueError("Nieprawidłowy zoom miniatur raportu.")
                if not math.isfinite(crop_m) or not (
                    core_config.REVIEW_CROP_M_MIN <= crop_m <= core_config.REVIEW_CROP_M_MAX
                ):
                    raise ValueError(
                        f"Zoom miniatur raportu musi być w zakresie {core_config.REVIEW_CROP_M_MIN:g}-{core_config.REVIEW_CROP_M_MAX:g} m."
                    )
            ...
            try:
                ...
                if "rank" in data:
                    ...
                    result = save_manual_wreck(data.get("lat"), data.get("lon"), core_config.WRECKS_DIR)
                self._send_json(200, result)
            except Exception as e:
                self._send_json(400, {"error": str(e)})
        else:

...
- [What is in v1.1](#what-is-in-v11)
...
- Interactive Leaflet map with Wrocław base imagery, Geoportal `STND` preview, `OSM` base map, scan crosshair, and result pins.
...
- Surface layer from OSM/Overpass for roads, sidewalks, parking areas, kerbs, and `surface` tags; it is off by default and loads only when enabled.
- Administrator panel with GeoTIFF cache map, disk limit, photo-review filters, and signed-out layer visibility settings.
...
- surface data: roads, sidewalks, parking, kerbs, and material from OSM/Overpass.

The base-layer slider includes `2020`-`2025`, `STND`, and `OSM`. Administrators can restrict which marker layers and base maps are visible to signed-out users.
...
- Each case directory contains `record.json`, a local `index.html`, and evidence packages with yearly thumbnails, a candidate snapshot, and analysis metadata.
...
- A vehicle-case pin has a delete action, which removes the local case directory and refreshes the map layer.
- When the model misses a vehicle, clicking inside the downloaded area opens **Manual location inspection** with historical thumbnails. The save action creates a manual `zidentyfikowane_wraki/<wreck_id>/` case directory at the clicked coordinates and marks it as `manual_inspection`.
...
- A user or administrator can add on-site photos directly to a vehicle case from its pin or from that case's public `index.html`. Photos have a private original and a public copy without EXIF. Only approved anonymized public copies are publicly visible.

The `zidentyfikowane_wraki/` directory is ignored by git.

## Field photos

Field photos are handled as a separate map layer. Users can add photos to the `pending` queue, and the administrator can approve, anonymize, move them to cases, and delete them; uploads are independent from vehicle cases and do not mix with YOLO analysis results.
...
- A field-photo or grouped-photo popup gives administrators a quick **Edit photo anonymization** action that opens the review queue scoped to the selected photos.
